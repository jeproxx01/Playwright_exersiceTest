...
test("handle popups",async({browser})=>{
    ...
    await Promise.all([page.waitForEvent('popup'), page.locator("#PopUp").click()]); //removed await
    ...
    expect(allPopupWindows).toHaveLength(3);
    ...
    //updated site no playwright popup exists. 
